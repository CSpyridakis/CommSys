function [periodogram_figure, theoritical_practical_SDP_figure] = SDP_theoretical_experimental(partA, partB, PAM, T, Ts, A, a, Nf, N, K, over)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Create periodogram and calculate theoretical SPD
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Compute Experimental SDP to compare it with theoretical
    for i = 1:K                                            
    ...
    % Display signals
    if (~isempty(partB)); 
        theoritical_practical_SDP_figure = figure();
        p1 = semilogy(F_Px, Px_F_experimental, 'b') ; hold on;
        p2 = semilogy(F_Px, Px_F_theoritical, 'r') ; hold off;
        legend([p1, p2],'Experimental', 'Theoretical'); legend('Location','NorthEast'); grid on; title([partB,' Theoretical-Experimental SDP ', num2str(PAM), '-PAM (K= ', num2str(K), ', N= ', num2str(N), ')']); ylabel('P_x(F)'); xlabel('F(Hz)');
    end
end