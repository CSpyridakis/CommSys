...
f=figure(); p_num=1;
  for i=1:length(a)             % for a = 0, 0.5, 1
    for k=[0 1 2 4]             % for k = 0,1,2,4
      ...
      plot(t_s, phi_t_za, 'r-', 'DisplayName','\phi(t)') ; hold on ;                        % φ(t) 
      plot(t_s, phi_kt_za, 'b-.', 'DisplayName', '\phi(t-kT)') ; hold on                    % φ(t-kT)
      plot(t_s, phi_t_za.*phi_kt_za,'m', 'DisplayName','\phi(t)*\phi(t-kT)') ; hold off;    % φ(t)*φ(t-kT) 
